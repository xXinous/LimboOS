...
import { LogOut, Info, X, ShieldAlert, ChevronUp } from 'lucide-react';
...
export const SecurityMenu = ({ onClose, onLogout, onShowMetadata }: SecurityMenuProps) => {
  ...
  return (
    <motion.div 
      ...
      className="absolute inset-0 z-[100] bg-cardboard/98 backdrop-blur-md flex flex-col border-b-[12px] border-cardboard-dark shadow-[0_25px_60px_rgba(0,0,0,0.8)] overflow-hidden cursor-grab active:cursor-grabbing touch-none"
    >
      {/* Camada Estética CRT (Scanlines e Brilho) */}
      <div className="absolute inset-0 pointer-events-none opacity-[0.04] z-10 scanlines" />
      <div className="absolute inset-0 pointer-events-none bg-gradient-to-b from-transparent via-analog-orange/5 to-transparent h-40 animate-scanline z-10" />
      ...
      <div className="flex-1 p-8 flex flex-col gap-6 justify-center relative z-20">
        ...
        >
          ...
          <div>
            <div className="text-xl font-oswald tracking-wider">Diagnóstico</div>
            <div className="text-[10px] opacity-60 font-mono tracking-tight mt-1">VERIFICAR INTEGRIDADE DA FITA</div>
          </div>
        </button>
      </div>

      {/* Aba de Puxar (Visual e Funcional) */}
      <div className="p-10 border-t-2 border-cardboard-dark bg-black/5 flex flex-col items-center gap-4 relative z-20 mt-auto">
         <div className="flex flex-col items-center gap-1 opacity-30">
           ...
           <span className="font-oswald text-xs tracking-[0.4em] uppercase">Deslize para fechar</span>
         </div>
