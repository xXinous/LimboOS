...
export default function LoginScreen({ onLogin }: LoginScreenProps) {
  ...
  const [loggedInProfile, setLoggedInProfile] = useState<PlayerProfile | null>(null);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const result = loginOrCreate(username.trim(), password);
    ...
    setLoggedInProfile(result);
    setIsSuccess(true);

    // Wait for success animation, then transition
    setTimeout(() => {
      onLogin(result);
    }, 1800);
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 overflow-hidden relative bg-surface">
      ...
      <div className="absolute top-0 left-0 w-full h-full opacity-[0.04] pointer-events-none select-none overflow-hidden">
        <div className="absolute top-8 left-8 font-display text-[12vw] font-bold leading-none tracking-tighter text-industrial-silver">
          TAC-01
        </div>
        <div className="absolute bottom-8 right-8 font-display text-[12vw] font-bold leading-none tracking-tighter text-industrial-silver text-right">
          SYS-86
        </div>
      </div>
              ...
              <div className="bg-surface p-8">
                ...
                <form onSubmit={handleLogin} className="space-y-8">
                  <div className="space-y-6">
                    {/* Username */}
                    <div className="group">
                      <label className="block text-[10px] font-display font-bold uppercase tracking-[0.2em] mb-2 text-industrial-silver/60 transition-colors group-focus-within:text-primary">
                        Codinome do Agente
                      </label>
                  ...
                  <div className="flex items-center justify-between gap-6 pt-2">
                    <p className="text-[9px] font-display text-industrial-silver/30 uppercase tracking-widest">
                      Novo agente? Crie um codinome.
                    </p>
                    <button
                      type="submit"
                      disabled={isLoggingIn}
                      className="flex-shrink-0 bg-primary hover:bg-primary-container text-black font-display font-bold text-xs uppercase tracking-[0.2em] py-4 px-8 transition-all flex items-center justify-center gap-3 group disabled:opacity-50 disabled:cursor-not-allowed glow-orange active:scale-95"
                    >
                      {isLoggingIn ? (
                        <>
              ...
              <div className="bg-surface-container-highest h-10 flex items-center px-8 justify-between">
                <div className="flex gap-1">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <div
                      key={i}
                      className={`w-1.5 h-3 ${isLoggingIn ? 'animate-pulse bg-primary' : 'bg-industrial-silver/20'}`}
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm font-display uppercase tracking-[0.3em] text-industrial-silver/60">
                Bem-vindo, {loggedInProfile?.username ?? 'Agente'}
              </p>
            </div>
          </motion.div>
        )}
      ...
      <div className="hidden lg:block absolute left-12 top-1/2 -translate-y-1/2 space-y-12">
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-display font-bold uppercase tracking-widest text-industrial-silver">
              Processador Central
            </span>
          </div>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 opacity-20">
        ...
        <div className="text-[10px] font-display font-bold uppercase tracking-[0.4em] text-industrial-silver">
